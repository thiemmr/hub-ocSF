...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    ...
    # 改为 while 循环，支持多轮连续工具调用，执行工具 → 将结果回填到 messages → 再次请求 LLM
    MAX_TOOL_TURNS = 5  # 安全锁，防止无限循环
    turn = 0

    while msg.tool_calls and turn < MAX_TOOL_TURNS:
        turn += 1
    ...
    # 如果因为达到最大轮次退出，而 msg 里依然有 tool_calls，可以给个提示
    if msg.tool_calls:
        print(f"⚠️ 达到最大工具调用轮次 ({MAX_TOOL_TURNS})，强制截断。")
