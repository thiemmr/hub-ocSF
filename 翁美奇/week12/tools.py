...
def _embed_query(text: str) -> np.ndarray:
    ...
    print(f"Embedding 维度: {vec.shape}, 范数: {np.linalg.norm(vec):.6f}")
...
def store_user_preference(preference: str) -> str:
    """
    存储用户的新偏好、事实或计划。
    当用户明确表达喜好、生活习惯、投资偏好或重要个人信息时调用此工具。
    """
    # 这个函数会在 AgentMemory 实例中调用，我们稍后注入
    # 注意：这里只是占位，实际逻辑在 run 中处理
    return f"已存储偏好: {preference}"
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "存储用户的新偏好、事实或计划",
            "parameters": {
                "type": "object",
                "properties": {
                    "preference": {"type": "string", "description": "用户偏好内容"}
                },
                "required": ["preference"]
            }
        }
