...
async def _stream_react(question: str, max_steps: int, mode: str):
    ...
    def _worker(agent_memory: AgentMemory):
        try:
            for step_data in react_run(question, max_steps=max_steps, agent_memory=agent_memory):
                queue.put_nowait(step_data)
        finally:
            queue.put_nowait(_SENTINEL)
    # 使用 FAISS 模式存储记忆信息
    agent_memory = AgentMemory(mode="faiss")
    
    yield _sse({"type": "start", "question": question, "mode": mode})
    loop = asyncio.get_event_loop()
    loop.run_in_executor(None, _worker, agent_memory)
