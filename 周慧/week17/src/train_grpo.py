...
PROJECT_ROOT = ROOT.parent.parent.parent            # /Users/zhouhui/practice（共享 pretrain_models 所在）
MODEL_PATH = PROJECT_ROOT / "pretrain_models" / "Qwen2-0.5B-Instruct"
...
def main():
    ...
    config = GRPOConfig(
        ...
        model_init_kwargs={"torch_dtype": "bfloat16", "attn_implementation": "eager"},
        # ── GRPO 核心参数 ─────────────────────────────────────────────
        ...
        bf16=torch.cuda.is_available(),   # MPS 不支持 accelerate 的 bf16 混合精度；权重已由 torch_dtype=bfloat16 落地
    ...
    if torch.cuda.is_available():
        ...
        print(f"MPS 峰值显存: {peak_gb:.2f} GB")
